...
class RobotController:
    ...
    def move_robot_joints(self):
        ...
        if ret[0] != 0:
            ...
            print("Robot moved to target pose controlled in joint space")
            self.reset()
            return True

    def robot_grasp_pre(self, Arm_stop_pre):
        print("Starting robot_grasp_pre...")
        if self.set_target_pre(Arm_stop_pre):
            print("Target set successfully. Starting movement...")
            return self.move_robot_joints()
    ...
    def move_robot_linear(self):
        ...
        #print(self.target_endPose)
        ret = self.robot.linear_move(self.target_endPose, 0, False, 30)
        if ret[0] != 0:
    ...
    def robot_linear_move(self, target_pose):
        print("Starting robot_linear_move...")
        if self.set_target_linear(target_pose):
            return self.move_robot_linear()
    ...
    def harvest_move(self):
        #time.sleep(2) #time for remove the support
        joint_increasing_angle = [0, 0, 0, 0, 0, -1.57]
        joint_decreasing_angle = [0, 0, 0, 0, 0, 1.57]

        ret = self.robot.joint_move(joint_increasing_angle, 1, False, 2)
        ret = self.robot.joint_move(joint_decreasing_angle, 1, False, 2)
        time.sleep(4)

        return True

    def boxing_move(self):
        ret = self.robot.joint_move(START_POSE, 0, False, 30) #return back to start pose
        joint_boxing = deg_to_rad([0, -15, 108, 0, 87, 0])
        ret = self.robot.joint_move(joint_boxing, 0, False, 10)
...
def create_info_image(detected_objects, selected_object_index):
    info_image = np.zeros((200, 640, 3), dtype=np.uint8)
