...
def display_thread(stop_event):
    ...
    dual_camera_processor = DualCameraProcessor(2, 4)  # Adjust camera indices as needed
    dual_camera_processor.start()

    while not stop_event.is_set():
        # Get processed frames from DualCameraProcessor
        frame1, frame2 = dual_camera_processor.get_processed_frames()
                        ...
                        if final_coords is not None and len(final_coords) >= 3:
                            ...
                            if robot_controller.robot_grasp_pre(Arm_stop_pre):
                                ...
                                if robot_controller.robot_linear_move(linear_target):
                                    ...
                                    if gripper_state in [1, 2, 3]:
                                        print("within the 1 or 2 or 3")

                                        gripper.set_position(1000)
                                        time.sleep(2)
                                        modified_Z = linear_target[2] - 10
                                        modified_linear_target = (*linear_target[:2], modified_Z, *linear_target[3:])
                                        robot_controller.robot_linear_move(modified_linear_target)
                                        time.sleep(1)
                                        gripper.set_position(430)
                                        time.sleep(2)

                                        gripper.set_position(1000)
                                        time.sleep(2)
                                        modified_Z = linear_target[2] - 15
                                        modified_linear_target = (*linear_target[:2], modified_Z, *linear_target[3:])
                                        robot_controller.robot_linear_move(modified_linear_target)
                                        time.sleep(1)
                                        gripper.set_position(370)
                                        time.sleep(2)
